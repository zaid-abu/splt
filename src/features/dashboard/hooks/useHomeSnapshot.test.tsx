...
describe("useHomeSnapshot", () => {
  it("returns undefined data while queries are loading", async () => {
    (useGroups as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useFriends as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useUserExpenses as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useUserSettlements as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useOpenBalances as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useNotifications as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useUserActivities as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
    (useRecurringExpenses as Mock).mockReturnValue(
      makeMockReturn(undefined, { isLoading: true, isFetched: false })
    );
  ...
  it("shows stale offline when data exists but a query errors", async () => {
    ...
    (useUserSettlements as Mock).mockReturnValue(makeMockReturn([]));
    (useOpenBalances as Mock).mockReturnValue(
      makeMockReturn([], { isError: true, error: new Error("Network error") })
    );
    (useNotifications as Mock).mockReturnValue(makeMockReturn([]));
    (useUserActivities as Mock).mockReturnValue(makeMockReturn([]));
    (useRecurringExpenses as Mock).mockReturnValue(makeMockReturn([]));
  ...
  it("refresh() calls refetch on every source", async () => {
    ...
    (useUserSettlements as Mock).mockReturnValue(
      makeMockReturn([], { refetch: refetchSettlements })
    );
    ...
    (useRecurringExpenses as Mock).mockReturnValue(
      makeMockReturn([], { refetch: refetchRecurring })
    );

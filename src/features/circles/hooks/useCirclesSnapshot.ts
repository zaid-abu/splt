...
export function useCirclesSnapshot(userId: string, search: string): SnapshotState<CirclesData> {
  ...
  const data = useMemo<CirclesData | undefined>(() => {
    ...
    const orderedBalances = orderBalances(openBalances, preferredCurrency.code).filter(
      (ob) => ob.signedAmountMinor !== 0
    );
    ...
    const pendingRequests: RequestItem[] = allFriendships
      .filter((f) => f.status === "pending" && f.friendId === userId && f.friendUser)
    ...
    const groupSections: GroupSection[] = groups.map((g) => {
      ...
      const allSameCurrency = groupBalances.every(
        (ob) => ob.currency === groupBalances[0].currency
      );
    ...
    const personSections: PersonSection[] = Array.from(personSet.values())
      ...
      .map((u) => {
        ...
        const sharedGroupCount = new Set(
          balances
            .filter((balance) => balance.context.type === "group")
            .map((balance) => balance.context.groupId)
        ).size;

        const allSameCurrency = balances.every(
          (ob) => ob.currency === (balances[0]?.currency ?? "")
        );
        let netSignedMinor = 0;
        ...
        const topBalance: OpenBalance | null =
          balances.length > 0
            ? {
                counterpartyId: u.id,
      ...
      .sort((a, b) => {
        ...
        const rankDiff = (rankOrder[a.classification] ?? 4) - (rankOrder[b.classification] ?? 4);

...
import { useReceiptUpload } from "@/features/expenses/hooks/useReceiptUpload";
...
import { ExpenseReceiptControl } from "@/features/expenses/components/ExpenseReceiptControl";

export default function NewExpenseFlow(): JSX.Element {
  ...
  const {
    isReceiptSheetVisible: showReceiptSheet,
    isUploadingReceipt,
    openReceiptSheet,
    closeReceiptSheet,
    pickImage: handlePickImage,
    pickPdf: handlePickPdf,
    removeReceipt: handleRemoveReceipt,
  } = useReceiptUpload({ operationId, currentReceipt: composer.receipt, setReceipt, toast });
  ...
  // Selecting a group or person is sufficient context; do not add a second confirmation step.
  useEffect(() => {
    if (hasSelection && !composer.context && participants.length > 0) {
      ...
      const currency = selectedGroup ? selectedGroup.currency : preferredCurrency.code;
  ...
  const todayDateString =
    new Date().toDateString() === composer.date.toDateString() ? "Today" : dateString;

  const contextName =
    selectedGroup?.name || selectedFriends.map((f: any) => f.name.split(" ")[0]).join(", ");
  const categoryLabel =
  ...
  return (
    <CoralScreen scroll={false}>
      <CoralTopBar title="Add expense" onBack={closeScreen} />
            ...
            {!hasContext ? (
              ...
              <View style={{ gap: 16, paddingTop: 22 }}>
                ...
                <Pressable
                  ...
                  accessibilityLabel={
                    showAdvancedOptions
                      ? "Hide advanced expense options"
                      : "Show advanced expense options"
                  }
                  accessibilityState={{ expanded: showAdvancedOptions }}
                  onPress={() => setShowAdvancedOptions((visible) => !visible)}
                  style={({ pressed }) => ({
                    flexDirection: "row",
                    alignItems: "center",
                    justifyContent: "space-between",
                ...
                >
                  <View>
                    ...
                    >
                      {splitMethodLabel} split · {splitCountLabel}
                    </Text>
                  </View>
                  {showAdvancedOptions ? (
                    <icons.ChevronUp size={18} color={coral.muted} />
                  ) : (
                    <icons.ChevronDown size={18} color={coral.muted} />
                  )}
                </Pressable>
                ...
                {showAdvancedOptions ? (
                  <View style={{ gap: 16 }}>
                    ...
                    <ExpenseReceiptControl
                      receipt={composer.receipt}
                      receiptLabel={receiptLabel}
                      isUploading={isUploadingReceipt}
                      onAdd={openReceiptSheet}
      ...
      <CoralSheet visible={showReceiptSheet} onClose={closeReceiptSheet}>

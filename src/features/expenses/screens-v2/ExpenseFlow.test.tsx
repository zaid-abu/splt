    ...
    data: {
      expense: {
        ...
        splits: [
          {
            userId: "me",
            user: { id: "me", name: "You", initials: "Y" },
            amount: 25,
            amountMinor: 2500,
            percentage: 50,
            shares: null,
          ...
          {
            userId: "a",
            user: { id: "a", name: "Alice", initials: "A" },
            amount: 25,
            amountMinor: 2500,
            percentage: 50,
            shares: null,
...
describe("ExpenseCreateSuccess component", () => {
  ...
  it("successful undo calls deleteExpense and onUndoSuccess", async () => {
    ...
    await render(
      React.createElement(ExpenseCreateSuccess, {
        ...baseProps,
  ...
  it("shows edit button when canEdit is true", async () => {
    ...
    mockSnapshot.mockReturnValue(
      makeSnapshotData({
        data: {
          ...makeSnapshotData().data,
          permissions: { canEdit: true, canDelete: false, deleteNeedsOwnerConfirmation: false },
        },
      })
    );
...
describe("ExpenseComments component", () => {
  ...
  const {
    useExpenseComments: mockUseExpenseComments,
  } = require("@/features/expenses/queries/useComments");
  ...
  it("shows delete button for comment author", async () => {
    mockUseExpenseComments.mockReturnValue({
      data: [
        { id: "c1", expenseId: "e1", userId: "me", text: "My comment", createdAt: new Date() },
      ],
      isLoading: false,
      isError: false,
      refetch: jest.fn(),
...
describe("EditExpenseScreen", () => {
  ...
  it("saves changes via updateExpense and navigates back", async () => {
    ...
    const {
      useUpdateExpense: useUpdateExpenseMock,
    } = require("@/features/expenses/queries/useExpenses");

...
import { aggregateOpenBalances, orderBalances } from "@/features/money/balances";
...
export function useGroupSnapshot(
  ...
  const isNotFound =
    !isLoadingAny && data === undefined && groupQuery.isFetched && !groupQuery.data;
  const isRestricted =
    data !== undefined && !data.permissions.canEdit && !data.permissions.canDelete;

...
export default function GroupDetailScreen(): JSX.Element {
  ...
  const filteredExpenses = expenses.filter((e) => {
    ...
    return e.title.toLowerCase().includes(q) || e.paidByUser?.name?.toLowerCase().includes(q);
  });
  ...
  return (
    <CoralScreen>
      ...
      <View style={{ marginTop: 8, marginBottom: 12 }}>
        <CoralSegment options={VIEW_OPTIONS} selected={view} onSelect={selectView} />
      </View>
        ...
        <>
          ...
          {nonZeroMembers.length > 0 ? (
            nonZeroMembers.map((member) => {
              ...
              const majorAmount = minorToMajor(
              ...
              return (
                <MoneyRow
                  ...
                  amountTone={
                    majorAmount > 0 ? "positive" : majorAmount < 0 ? "negative" : "neutral"
                  }
                ...
                {monthExpenses.map((expense) => {
                  ...
                  return (
                    <MoneyRow
                      ...
                      title={expense.title}
                      subtitle={
                        isPayer ? `${dateLabel} - You paid` : `${dateLabel} - ${payerName} paid`
                      }
                      amount={formatSignedAmount(balanceImpact, expense.currency)}
                      amountTone={
                        balanceImpact > 0 ? "positive" : balanceImpact < 0 ? "negative" : "neutral"
      ...
      {view === "schedule" && (
        <>
          {(!CAPABILITIES.scheduledExpenseReview ||
            !scheduleData.needsReview ||
          ...
          (!scheduleData.paused || scheduleData.paused.length === 0) ? (
            ...
            <>
              {CAPABILITIES.scheduledExpenseReview && scheduleData.needsReview.length > 0 && (
                <>
          ...
          <View style={{ marginTop: 20 }}>
            <CoralButton label="Schedule expense" variant="primary" onPress={handleSchedulePress} />
          </View>
        </>
      )}
    </CoralScreen>

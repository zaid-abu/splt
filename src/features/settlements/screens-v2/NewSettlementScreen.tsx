    ...
    for (const group of candidates) {
      ...
      const latest = latestBalances.find((balance) => {
        ...
        if (balance.context.type === "direct" && entry.context.type === "direct") {
          ...
          return Boolean(
            latestFriendshipId && entryFriendshipId && latestFriendshipId === entryFriendshipId
          );
        }
        return false;
      });
    ...
    <CoralScreen>
      ...
      {settleResult === "success" ? (
        ...
        <>
          ...
          {isError ? (
            <View
              style={{ minHeight: 280, alignItems: "center", justifyContent: "center", gap: 14 }}
            >
              <Text
                style={{
                  fontFamily: "InstrumentSans_600SemiBold",
          ...
          >
            ...
            >
              {formatAmount(
                minorToMajor(
                  candidates.reduce(
                    (s, g) => s + Math.abs(g.entries[0]?.signedAmountMinor ?? 0),
                    0
                  ),

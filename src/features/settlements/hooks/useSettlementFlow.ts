...
export function settlementFlowReducer(
  state: SettlementFlowState,
  action: SettlementFlowAction
): SettlementFlowState {
  switch (action.type) {
    case "START_COMPOSE":
      return {
        ...
        amountInput: minorToMajor(
          Math.abs(action.selection.signedAmountMinor),
          action.selection.currency
        ).toString(),
        method: "cash",
        note: "",
    ...
    case "UPDATE_BALANCE":
      ...
      return {
        ...
        amountInput: minorToMajor(
          Math.abs(action.signedAmountMinor),

...
function composeState(
  overrides: Partial<SettlementFlowState & { step: "compose" }> = {}
): SettlementFlowState & { step: "compose" } {
  return {
    step: "compose",
...
describe("settlementFlowReducer", () => {
  ...
  describe("GO_TO_REVIEW", () => {
    ...
    it("does not transition when amount is zero", () => {
      const next = settlementFlowReducer(composeState({ amountInput: "0" }), {
        type: "GO_TO_REVIEW",
      });
      expect(next.step).toBe("compose");
    });
    ...
    it("does not transition with empty amount", () => {
      const next = settlementFlowReducer(composeState({ amountInput: "" }), {
        type: "GO_TO_REVIEW",
      });
      expect(next.step).toBe("compose");
    });

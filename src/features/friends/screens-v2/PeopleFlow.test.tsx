...
jest.mock("@/features/navigation/phase2Routes", () => ({
  expenseHref: (ctx?: any) =>
    ctx?.type === "direct"
      ...
      : "/expense/new",
  settlementHref: (input?: any) =>
    input?.context?.type === "direct"
      ? {
          pathname: "/settle/[id]",
          params: { id: input.context.friendshipId, friendshipId: input.context.friendshipId },
        }
      : "/settle/new",
...
describe("add flow", () => {
  ...
  it("shows self state when searching own email", async () => {
    mockSearchFriends.mockResolvedValueOnce({
      state: "found",
      userId: "me",
      name: "Me",
      initials: "M",
...
describe("invite redemption flow", () => {
  ...
  it("shows valid invite with accept button", async () => {
    resolveFriendInvite.mockResolvedValueOnce({
      state: "valid",
      inviterId: "u-inviter",
  ...
  it("shows self state", async () => {
    ...
    expect(await findByText("Your Own Invite")).toBeTruthy();
    expect(await findByText("You cannot accept your own invite.")).toBeTruthy();
  });
...
describe("share invite", () => {
  it("creates invite and shares raw token", async () => {
    createFriendInvite.mockResolvedValueOnce({
      inviteId: "inv-1",
      rawToken: "tok_shared",
      expiresAt: new Date("2026-12-31"),
    });
...
describe("detail screen", () => {
  ...
  it("shows loading state", async () => {
    mockUsePersonSnapshot.mockReturnValue(
      baseSnapshot({ data: undefined, isInitialLoading: true })
    );
  ...
  it("remove allowed when settled alerts confirmation", async () => {
    ...
    expect(Alert.alert).toHaveBeenCalledWith(
      "Remove Friend?",
      expect.any(String),
      expect.arrayContaining([expect.objectContaining({ text: "Remove", style: "destructive" })])
    );
  });

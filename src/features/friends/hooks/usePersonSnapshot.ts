...
export function usePersonSnapshot(counterpartyId: string): SnapshotState<PersonSnapshotData> {
  ...
  const data = useMemo<PersonSnapshotData | undefined>(() => {
    ...
    const sharedExpenses = expenses.filter((exp) => {
      const involvesPerson =
        exp.paidBy === counterpartyId || exp.splits.some((s) => s.userId === counterpartyId);
      const involvesMe =
        exp.paidBy === currentUser.id || exp.splits.some((s) => s.userId === currentUser.id);
    ...
    const sharedSettlements = settlements.filter((set) => {
      const involvesPerson = set.fromUserId === counterpartyId || set.toUserId === counterpartyId;
      const involvesMe = set.fromUserId === currentUser.id || set.toUserId === currentUser.id;
      return involvesPerson && involvesMe;
    });
    ...
    localActivities.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

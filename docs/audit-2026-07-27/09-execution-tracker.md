...
| ----- | -------------------------------------------------------- | ------- | ------------------ |
| P0    | Reliable gates; false UI contained                       | PLANNED | P0-12              |
| P1    | Ledger writes are authorized and internally consistent   | PLANNED | P1-14              |
...
| ----- | ------ | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| P0-01 | TODO   | —                       | Record the current dirty-worktree baseline, classify existing changed/untracked files as pre-existing vs program-owned, and create no destructive cleanup.                                                                        | `git status --short`; evidence log contains the baseline and overlap risks                           |
| P0-02 | TODO   | P0-01                   | Separate TypeScript ownership for RN app, Jest, Playwright, and Supabase functions so Node types do not enter the app bundle and every config typechecks.                                                                         | Run each configured typecheck plus `npm run typecheck`                                               |
| P0-03 | TODO   | P0-01                   | Exclude Playwright reports, test results, coverage, Expo output, and generated traces from lint/format/source control without hiding owned source.                                                                                | `npm run lint` output contains no generated report assets; `git status --ignored --short` spot-check |
| P0-04 | TODO   | P0-02,P0-03             | Fix all scoped source lint errors, including effect-state and React Compiler memoization issues, without disabling rules globally.                                                                                                | `npx eslint src e2e playwright.config.ts --quiet`                                                    |
| P0-05 | TODO   | P0-01                   | Repair Circles and CircleDock test imports/mocks and centralize router/Reanimated mocks where appropriate.                                                                                                                        | Targeted suites pass; no skipped assertions added                                                    |
| P0-06 | TODO   | P0-01                   | Repair MoneyMap/worklets test initialization using a reusable supported Jest setup.                                                                                                                                               | MoneyMap suite passes; full Jest run does not fail on worklets initialization                        |
| P0-07 | TODO   | P0-04,P0-05,P0-06       | Restore a green full unit/component baseline and record suite/test counts.                                                                                                                                                        | `npm run test -- --runInBand`                                                                        |
| P0-08 | TODO   | P0-04                   | Format owned source as a dedicated reviewable change; do not reformat migrations or user work blindly.                                                                                                                            | `npm run format:check`; diff reviewed                                                                |
| P0-09 | TODO   | P0-01                   | Add a typed capability registry and hide scheduled review, fake export, fake biometric/session/delete, no-op notification/motion, and inert help controls until implemented. No reachable control may lie or silently do nothing. | Route/control tests plus manual route inventory                                                      |
| P0-10 | TODO   | P0-02,P0-03,P0-07,P0-08 | Make CI run install, generated drift, typecheck, scoped lint, format, Jest, and local database tests in clear jobs with useful artifacts.                                                                                         | Validate workflow syntax; run equivalent commands locally                                            |
| P0-11 | TODO   | P0-09                   | Perform baseline iOS and Android walkthroughs for every live primary route; record screen, action, state, accessibility, and data defects as `DISC-*` rows.                                                                       | Evidence links to device/build/OS notes and captures                                                 |
| P0-12 | TODO   | P0-07,P0-09,P0-10,P0-11 | Phase review: prove all required gates are green and no reachable control is inert or deceptive; update baseline counts and phase status.                                                                                         | Review checklist; full Phase 0 command set                                                           |
...
| ID    | Status | Depends on              | Work and acceptance criteria                                                                                                                                       | Verification                                                        |
| ----- | ------ | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| P1-01 | TODO   | P0-12                   | Write ADRs for money/currency transport, expense authorization, split authority, and settlement reversal/audit semantics. Resolve safe JS transport limits.        | ADR review; decision log updated                                    |
| P1-02 | TODO   | P1-01                   | Add failing pgTAP authorization cases for anonymous/outsider/removed/pending users, foreign payer, injected/duplicate participants, and invalid contexts.          | Tests fail for the intended missing checks before implementation    |
